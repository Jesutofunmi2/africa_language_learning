            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-8 offset-2 mr-auto">
                    <div class="card card-signup text-center">
                        ...
                        <div class="card-body ">
                            <form class="form" method="POST" action="{{ route('admin.option.update', $option->id) }}"
                                ...
                                <div class="input-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input name="title" type="text" class="form-control" placeholder="Title"
                                        value="{{ old('title') ?? $option->title }}" required autofocus>
                                    @if ($errors->has('title'))
                                ...
                                <div class="row-12">
                                    <div class="form-group">
                                        <select name="language_id" class="form-control">
                                            <option value="{{ old('language_id') ?? $option->language->id }}">
                                ...
                                <div class="row-12">
                                    <div class="form-group">
                                        <select name="question_id" class="form-control">
                                            <option value="{{ old('question_id') ?? ($option->question->id ?? null) }}">
                                                {{ $option->question->title ?? null }}</option>
                                ...
                                <input type="file" value="{{ old('image_url') }}" name="image_url"
                                    placeholder="Select Audio/Video" accept="image/*" class="form-control" />
                                @if ($errors->has('image_url'))
                                    <span class="invalid-feedback" style="display: block; border:30px" role="alert">
                                        <strong>{{ $errors->first('image_url') }}</strong>
                                    </span>
                                @endif
                                <img src="{{ asset($option->image_url) }}" width="200px" height="200px" alt=""
                                    srcset="">
