var cal = {
  ...
  show : (el) => {
    ...
    date_input = document.getElementById('a_date');
    cal.sDay = el.getElementsByClassName("dd")[0].innerHTML;
    let = picked_date = document.getElementById('evt-date');
    picked_date.innerHTML = `${cal.sDay} ${cal.mName[cal.sMth]} ${cal.sYear}`;
    let add_one = localStorage.getItem('add_one');
    if(!add_one) cal.sMth++;
    m = cal.sMth < 10 ? `0${cal.sMth}` : cal.sMth;
    d = cal.sDay < 10 ? `0${cal.sDay}` : cal.sDay;
    let a_date = `${cal.sYear}-${m}-${d}`;

    date_input.value = a_date;
    
    document.getElementById("form-container").scrollIntoView(true);
    localStorage.setItem(`add_one`, true);
  },
